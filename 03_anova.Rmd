...
Y en este caso pondremos la restricción de que la suma de los $i$ parámetros $\alpha$ y los $j$ parámetros $\beta$ sean cero. Es decir $$\sum_{i}\alpha_i=\sum_{j}\beta{j}=0$$
...
### Modelo aditivo con iteracción

Este modelo es una extensión del modeo aditivo en donde adicional a suponer que los efectos de los dos factores se suman,  también se considera la presencia del efecto interacción npor lo que el modelo matemático queda de la siguiente forma:
...
$$\sum_{i}\alpha_i=\sum_{j}\beta{j}=\sum_{i}\gamma_{ij}=\sum_{j}\gamma_{ij}=0$$
...
Y con esto el modelo puede ser reparamentrizado de la siguiente forma:
...
|   Interacción  |        $b-1$       | $SC_{I}=r\sum_{ij}(y_{ij.}-y_{i..}-y_{.j.}+\overline{y})^2$ |$\frac{SC_{I}}{(a-1)(b-1)}$| $\frac{SC_{I}/[(a-1)(b-1)]}{SCR/[ab(r-1)]}$ |
